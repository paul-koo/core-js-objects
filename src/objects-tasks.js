...
function mergeObjects(objects) {
  const copy = {};
  objects.forEach((obj) => {
    const arrKeyObj = Object.keys(obj);
    arrKeyObj.forEach((key) => {
      if (copy[key]) {
        copy[key] += +obj[key];
      } else {
        copy[key] = obj[key];
...
function makeWord(lettersObject) {
  const arrWord = [];
  const keyObject = Object.keys(lettersObject);
  keyObject.forEach((key) => {
    lettersObject[key].forEach((element) => {
      arrWord[element] = key;
    });
  });
  return arrWord.toString().replaceAll(',', '');
}
